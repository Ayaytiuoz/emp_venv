...
class Tacheafaire extends Component{
    constructor(props) {
        super(props)
        this.state=
            {
    ...
    getTache=()=>{
        axios.get('http://127.0.0.1:8000/Task_Management/api/tache/en_attent').then(response =>{
    ...
    getPUT(id){
        var api ="http://127.0.0.1:8000/Task_Management/api/tache/encours/"+id
        fetch(api,{
            method:'PUT'
        }).then((result)=>{
                    ...
                    <table className="table table-hover">
                        ...
                        {this.state.Tache.map(taches =>(
                            <tr className="table-secondary">
                                ...
                                <td>
                                    <button onClick={()=>this.getPUT(taches.id_tache)} type="button" className="btn btn-outline-light">Lancer l'execution</button></td>
